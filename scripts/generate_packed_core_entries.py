...
def parse_entries(path: Path) -> list[Entry]:
    ...
    for match in re.finditer(
        ...
        matches.append((match.start(), Entry(parse_string(match.group(1)), match.group(2), match.group(3),
                                             parse_string(match.group(4)))))
...
def generate(entries: list[Entry]) -> str:
    ...
    for _, entry in ordered:
        ...
        records.append(f"    {{{surface_offset}, {lemma_offset}, static_cast<uint8_t>({pos}), "
